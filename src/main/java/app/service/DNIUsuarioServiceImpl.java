...
import static app.security.SecurityUtils.currentUserId;
...
public class DNIUsuarioServiceImpl implements IDNIUsuarioService{
    ...
    public void subirFrente(MultipartFile file) {
        ...
        // si estaba RECHAZADO, lo reseteo a REGISTRADO hasta que envíe
    ...
    public String getFrenteUrl(Long usuarioIdSolicitado) {
        Long me = currentUserId();
        mustBeOwnerOrAdmin(usuarioIdSolicitado, me);
    ...
    public void validar(Long usuarioId) {
        ...
        // opcional: exigir que esté en PENDIENTE
        // if (u.getNivelUsuario() != Usuarios.NivelUsuario.PENDIENTE) throw new RuntimeException("Estado inválido");
    ...
    private void mustBeAdmin() {
        ...
        if (!isAdmin) throw new org.springframework.security.access.AccessDeniedException("Solo ADMIN");
    }

    private void mustBeOwnerOrAdmin(Long targetUserId, Long me) {
        var auth = org.springframework.security.core.context.SecurityContextHolder.getContext().getAuthentication();
        boolean isAdmin = auth != null && auth.getAuthorities().stream()
                .map(org.springframework.security.core.GrantedAuthority::getAuthority)
                .anyMatch("ROL_ADMIN"::equals);
        if (!(isAdmin || targetUserId.equals(me))) {
